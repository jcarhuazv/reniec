<?php
	...
	class Reniec
	{
		...
		/* SearchPadron
		 * 
		 * Realiza busqueda de datos enviado peticiones a la pagina de reniec (Padron de Electores)
		 * 
		 * @param : string $dni 		CUI o numero de DNI
		 * 
		 * @return: array|false 		Array con datos encontrados en Reniec o false
		 * */
		function searchPadron( $dni )
		{
			$rtn=array();
			if( $dni != "" && strlen( $dni ) == 8 )
			{
		...
		function searchReniec( $dni )
		{
			...
			if( $dni != "" && strlen( $dni ) == 8 && $Captcha != false )
			{
				...
				$this->cc->setReferer("https://cel.reniec.gob.pe/valreg/valreg.do");
				...
				if( isset($matches[0]) )
				{
					$rtn["Nombres"]  = utf8_encode($matches[0][1]);
					$rtn["Apellidos"] = utf8_encode($matches[0][2])." ".utf8_encode($matches[0][3]);
					$rtn["Paterno"] = utf8_encode($matches[0][2]);
					$rtn["Materno"] = utf8_encode($matches[0][3]);
					$rtn["gvotacion"] = null;
					$rtn["Distrito"] = null;
					$rtn["Departamento"] = null;
				}
